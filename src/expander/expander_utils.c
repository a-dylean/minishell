...
char	*get_env_var_value(t_env *env_list, char *name, char *d_quoted)
{
	t_env	*temp;
...
char	*get_new_str_value(char *str, char *old_value, char *new_value, int prev_index)
{
	...
	while (str[i] && j <= size)
	{
		if (!ft_strncmp(str + i, old_value, ft_strlen(old_value)) && i == prev_index)
		{
			...
			ft_strlcpy(new_str + j, str + i + ft_strlen(old_value), ft_strlen(str));
			return (new_str);
		}
		new_str[j++] = str[i++];
	}
...
char	*get_value_after_expansion(char *str, t_shell *shell, int *i)
{
	...
	if (!var_name)
		return (free(str), NULL);
	value = get_env_var_value(shell->env_list, var_name, d_quoted);
	if (!value)
